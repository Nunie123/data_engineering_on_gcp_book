...
And that's it. We now have a Dataproc Cluster running Spark. Now let's submit a job and have actually do some work.
...
> gcloud dataproc jobs submit pyspark \
    ...
    --region=us-central1
```
We can see our transformed file:
``` bash
...
t_create_cluster.set_upstream(t_submit_job)
```
...
## Cleaning Up
We've already deleted our Dataproc cluster, so all that's left is for us to delete the bucket we created and the buckets created by Dataproc:
``` bash
> gsutil ls
